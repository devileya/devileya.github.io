...
function initSlideshow(slideshow) {
  ...
  setInterval(() => {
    const firstImage = [...slideshow.children].reduce((prev, current) =>
...
function refreshPerspective(slideshow) {
  ...
  // Translate each image accordingly
  {
    ...
    for (const [i, image] of [...slideshow.children].entries()) {
      ...
      let [y, z] = [
        Math.floor(image.dataset.y * perspectiveThreshold) -
          perspectiveThreshold / 2,
      ...
      while (
        perspective > 100 &&
        Math.abs(prevZ - z) < perspectiveThreshold * 0.2
      ) {
